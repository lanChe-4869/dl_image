...
'''
输出结构:
output_crop/images  存放成功裁切图
output_crop/_json   存放成功裁切对应 JSON
debug/process_ok    存放成功标框 debug 图
debug/process_fail/origin  复制失败原图，input 原始数据保持不动
debug/process_fail/debug   存放失败标框 debug 图
'''
...
def get_detections(model, image_rgb, conf_thres) -> List[Detection]:
    ...
    """
    results = model.predict(source=image_rgb, conf=conf_thres, verbose=False)
    if not results:
        return []
...
def round_int_box(box, img_w=None, img_h=None):
    """把 float box 转成 int box，并可选限制在图像范围内。"""
    x1, y1, x2, y2 = [int(round(float(v))) for v in box]
    if img_w is not None:
        x1 = int(clamp(x1, 0, img_w))
        x2 = int(clamp(x2, 0, img_w))
    if img_h is not None:
        y1 = int(clamp(y1, 0, img_h))
        y2 = int(clamp(y2, 0, img_h))
    return [x1, y1, x2, y2]


def box_to_crop_coords(box, crop_box):
    """把原图坐标系 box 转成裁切图坐标系，并截断到裁切图范围内。"""
    bx1, by1, bx2, by2 = [int(round(float(v))) for v in box]
    cx1, cy1, cx2, cy2 = [int(round(float(v))) for v in crop_box]

    crop_w = max(0, cx2 - cx1)
    crop_h = max(0, cy2 - cy1)

    return [
        int(clamp(bx1 - cx1, 0, crop_w)),
        int(clamp(by1 - cy1, 0, crop_h)),
        int(clamp(bx2 - cx1, 0, crop_w)),
        int(clamp(by2 - cy1, 0, crop_h)),
    ]
...
def save_crop_json(
    ...
    """保存与裁切结果对应的 JSON，坐标均为整数。"""
    json_path = Path(json_path)
    json_path.parent.mkdir(parents=True, exist_ok=True)

    crop_box_int = round_int_box(crop_box, img_w, img_h)
    crop_w = crop_box_int[2] - crop_box_int[0]
    crop_h = crop_box_int[3] - crop_box_int[1]

    main_people = []
    main_people_in_crop = []

    for idx, det in enumerate(selected_person_dets, start=1):
        person_box = round_int_box(det.safe_box, img_w, img_h)
        label = f"person_{idx}"
        ...
        main_people_in_crop.append({
            "id": idx,
            "label": label,
            "box": box_to_crop_coords(person_box, crop_box_int),
        })

    if selected_person_dets:
        confidence = float(min(d.conf for d in selected_person_dets))
    else:
        confidence = 0.0

    object_count = len(object_dets or [])
    if not notes:
        notes = f"YOLO crop result. protected_objects={object_count}"

    data = {
        "image_width": int(img_w),
        "image_height": int(img_h),
        "main_people": main_people,
        "union_box": round_int_box(union_box, img_w, img_h),
        "recommended_crop_box": crop_box_int,
        "best_aspect_ratio": str(ratio_name),
        "crop_width": int(crop_w),
        "crop_height": int(crop_h),
        "used_padding": float(used_padding),
        "main_people_in_crop": main_people_in_crop,
...
def draw_debug_image(
    ...
    label(label_text[:160], (16, 16), status_color)
...
def process_one_image(
    ...
    main_people_union_box = union_boxes(selected_person_boxes)
    ...
    protect_box = union_boxes(all_protected_boxes)
    ...
    ratio_name = "free"
    crop_box = protect_box
    if adjust_aspect:
        target_aspect, ratio_name = choose_closest_feasible_aspect(
            crop_box, allowed_ratios, img_w, img_h
        )
        if target_aspect is not None:
            crop_box = adjust_box_to_aspect_safe(crop_box, target_aspect, img_w, img_h)
        else:
    ...
    if json_path is not None:
        has_masks_for_json = any(d.mask_box is not None for d in selected_person_dets)
        json_notes = (
            f"generated_by_yolo; mode={'mask' if has_masks_for_json else 'bbox'}; "
            f"protected_objects={len(object_dets)}"
        )
        save_crop_json(
            json_path=json_path,
            img_w=img_w,
            img_h=img_h,
            selected_person_dets=selected_person_dets,
            union_box=main_people_union_box,
            crop_box=crop_box,
            ratio_name=ratio_name,
            used_padding=(used_padding_value if used_padding_value is not None else padding),
            object_dets=object_dets,
            notes=json_notes,
        )
    ...
    return f"cropped_{ratio_name}_{mode}{object_suffix}"
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, required=True, help="输出根目录。脚本会自动创建 output_crop/images、output_crop/_json、debug/process_ok、debug/process_fail/origin、debug/process_fail/debug")
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="可选：debug 根目录。默认使用 output_dir/debug，并自动分 process_ok / process_fail/origin / process_fail/debug。",
    )
    parser.add_argument(
        "--fallback",
        type=str,
    ...
    output_crop_root = output_root / "output_crop"
    output_images_dir = output_crop_root / "images"
    output_json_dir = output_crop_root / "_json"
    ...
    for img_path in tqdm(image_paths, desc="Cropping images"):
        ...
        out_path = output_images_dir / rel_path
        json_path = (output_json_dir / rel_path).with_suffix(".json")
        debug_ok_path = (debug_ok_dir / rel_path).with_suffix(".jpg")
        debug_fail_rel_path = rel_path.with_suffix(".jpg")
        debug_fail_path = (debug_fail_debug_dir / debug_fail_rel_path).with_suffix(".jpg")
        ...
        try:
            status = process_one_image(
                ...
                json_path=json_path,
                used_padding_value=args.padding,
        ...
        if status.startswith("failed") or status == "error":
            fail_origin_path = debug_fail_origin_dir / rel_path
            fail_origin_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(img_path, fail_origin_path)

        stats[status] = stats.get(status, 0) + 1

    print("\nDone.")
    print("Output folders:")
    print(f"  cropped images: {output_images_dir}")
    print(f"  crop json:      {output_json_dir}")
    print(f"  debug ok:       {debug_ok_dir}")
    print(f"  fail origin:    {debug_fail_origin_dir}")
    print(f"  fail debug:     {debug_fail_debug_dir}")
    print("Stats:")
...
PowerShell example:
1. 默认:
python ./process/04_image_crop_seg_v3_final_output_json.py `
