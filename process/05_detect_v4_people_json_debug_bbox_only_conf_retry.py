...
def build_conf_candidates(args) -> List[float]:
    """
    构造逐级降 conf 的检测阈值序列。
    默认：0.60 -> 0.50 -> 0.40 -> 0.30 -> 0.20。
    """
    start = float(args.conf)
    min_conf = float(args.conf_min)
    step = abs(float(args.conf_step))

    if args.disable_conf_retry:
        return [round(start, 4)]

    if step <= 0:
        raise ValueError("--conf_step must be > 0")

    # 如果用户传入的起始 conf 已经低于 min_conf，就只跑一次，避免生成反向序列。
    if start <= min_conf:
        return [round(start, 4)]

    values = []
    cur = start
    while cur >= min_conf - 1e-9:
        values.append(round(cur, 4))
        cur -= step

    # 确保 min_conf 一定被尝试到，避免浮点误差或 step 不整除。
    if values[-1] > min_conf + 1e-9:
        values.append(round(min_conf, 4))
...
def process_image(
    ...
    json_root: Path,
    debug_root: Path,
    fail_origin_root: Path,
    fail_json_root: Path,
    fail_debug_root: Path,
    model,
    args,
):
    ...
    conf_candidates = build_conf_candidates(args)
    final_result = None
    tried_confs = []

    # 从高 conf 到低 conf 逐级尝试；一旦满足双人主体要求，就立刻采用该结果。
    for conf_value in conf_candidates:
        tried_confs.append(conf_value)
    ...
    raw_person_dets = final_result["raw_person_dets"]
    unique_person_dets = final_result["unique_person_dets"]
    selected_person_dets = final_result["selected_person_dets"]
    main_ok = final_result["main_ok"]
    main_reason = final_result["main_reason"]
    used_conf = final_result["conf"]
    notes = final_result["notes"]
    required_people = final_result["required_people"]

    if len(tried_confs) > 1 and main_ok:
        retry_note = f"used lower conf {used_conf:.2f} after retry"
        notes = f"{notes}; {retry_note}" if notes else retry_note
    elif len(tried_confs) > 1 and not main_ok:
        retry_note = f"failed after conf retry {tried_confs[0]:.2f}->{tried_confs[-1]:.2f}"
        notes = f"{notes}; {retry_note}" if notes else retry_note
    ...
    if args.include_extra_fields:
        data["_extra"] = {
            ...
            "used_conf": used_conf,
            "tried_confs": tried_confs,
            "imgsz": args.imgsz,
            "required_people": required_people,
        }

    # 成功：只写入主 json/debug。
    # 失败：不写入主 json/debug，只写入 fail/json、fail/debug，并复制原图到 fail/origin。
    if status == "ok":
        json_path = (json_root / rel_path).with_suffix(".json")
        debug_path = (debug_root / rel_path).with_suffix(".jpg")
    else:
        json_path = (fail_json_root / rel_path).with_suffix(".json")
        debug_path = (fail_debug_root / rel_path).with_suffix(".jpg")
...
def main():
    parser = argparse.ArgumentParser(description="对已裁剪图片重新做人像检测。成功结果写入 json/debug；失败时逐级降低 conf 重试，最终失败才写入 fail/origin、fail/json、fail/debug。")
    ...
    parser.add_argument("--fail_dir", type=str, default=None, help="可选：单独指定 fail 输出目录。默认 output_dir/fail，里面会有 origin、json、debug 三个子目录；fail 不再写入主 json/debug")

    parser.add_argument("--model", type=str, default="yolov8n-seg.pt", help="YOLO 模型路径或模型名。可用 seg 模型，但本脚本只使用 bbox，不使用 mask_box")
    parser.add_argument("--conf", type=float, default=0.60, help="起始 YOLO 检测置信度阈值。默认 0.60；如果失败会按 --conf_step 逐级降到 --conf_min")
    parser.add_argument("--conf_min", type=float, default=0.20, help="conf 重试的最低阈值。默认 0.20")
    parser.add_argument("--conf_step", type=float, default=0.10, help="每次失败后 conf 降低的步长。默认 0.10")
    parser.add_argument("--disable_conf_retry", action="store_true", help="关闭 conf 逐级重试，只使用 --conf 单次检测")
    ...
    fail_json_root.mkdir(parents=True, exist_ok=True)
    ...
    for img_path in tqdm(image_paths, desc="Detecting people"):
        ...
        try:
            ...
            # 异常时也归到 fail：复制原图，并写一个最小 error JSON，方便后续校正和排查。
            try:
                copy_file_preserve_path(img_path, fail_origin_root / rel_path)
                error_json_path = (fail_json_root / rel_path).with_suffix(".json")
                save_json({
                    "name": str(rel_path).replace("\\", "/"),
                    "image_width": 0,
                    "image_height": 0,
                ...
                print(f"[WARN] failed to write error fail files: {copy_e}")
        stats[status] = stats.get(status, 0) + 1

    print("\nDone.")
    print(f"OK JSON output:     {json_root}")
    print(f"OK Debug output:    {debug_root}")
    print(f"Fail origin output: {fail_origin_root}")
    print(f"Fail JSON output:   {fail_json_root}")
...
python ./process/05_detect_v4_people_json_debug_bbox_only_conf_retry.py `
  --input_dir ./images/process_image/20260616_output_1/equal_2_1_test/equal_2_1_fail_5/output_crop `
  --output_dir ./images/process_image/detect/20260616_output_1_detect_v4/equal_2_1_test `
