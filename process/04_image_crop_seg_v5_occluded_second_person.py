...
def normalized_person_center_distance(a, b):
    """
    两个人框中心距离的归一化值。
    数值越小，说明两个框越靠近；用于判断小框是不是贴近主体的半身/遮挡人物。
    """
    ax, ay = box_center(a)
    bx, by = box_center(b)
    ...
    # 用较小人物框尺度做归一化。遮挡半身人通常框小，但中心仍靠近主体。
...
def is_likely_occluded_second_person(
    ...
    """
    判断面积偏小的 person 是否更像“半身/被遮挡的第二主体”，而不是背景路人。

    通过条件：
    1. 面积不能小到几乎是远处路人；
    2. 和最大主体距离较近；
    3. 与最大主体在纵向有重叠，或底部高度接近。

    这样可以让被挡住下半身、只露上半身/半身的人进入双人裁切。
    """
...
def select_main_person_detections(
    ...
    改进点：
    - 正常主体：仍然用面积比例过滤背景小人；
    - 半身/遮挡主体：即使面积不足，只要和最大主体距离近、纵向关系像合照，也允许进入主体候选。
    """
...
def process_one_image(
    ...
    allow_occluded_second_person=True,
    occluded_min_abs_area_ratio=0.002,
    occluded_max_center_distance=1.8,
    occluded_min_vertical_overlap=0.25,
):
    ...
    main_ok, selected_person_dets, main_reason = select_main_person_detections(
        ...
        allow_occluded_second_person=allow_occluded_second_person,
        occluded_min_abs_area_ratio=occluded_min_abs_area_ratio,
        occluded_max_center_distance=occluded_max_center_distance,
        occluded_min_vertical_overlap=occluded_min_vertical_overlap,
    )
...
def main():
    ...
    parser.add_argument(
        "--disable_occluded_second_person",
        action="store_true",
        help="关闭半身/遮挡第二主体兜底判断。默认开启，用于减少 small_second_person 误判。",
    )
    parser.add_argument(
        "--occluded_min_abs_area_ratio",
        type=float,
        default=0.002,
        help="半身/遮挡第二主体兜底的最低绝对面积比例。默认 0.002，比 min_main_abs_area_ratio 更宽松。",
    )
    parser.add_argument(
        "--occluded_max_center_distance",
        type=float,
        default=1.8,
        help="半身/遮挡第二主体与最大主体的最大归一化中心距离。越大越容易保留贴近的小人，默认 1.8。",
    )
    parser.add_argument(
        "--occluded_min_vertical_overlap",
        type=float,
        default=0.25,
        help="半身/遮挡第二主体与最大主体的最小纵向重叠比例。越小越宽松，默认 0.25。",
    )
    parser.add_argument(
...
PowerShell example:
1. 默认:
python ./process/04_image_crop_seg_v5_occluded_second_person.py `
  --input_dir ./images/process_image/20260616_output_1/equal_2_1/debug/process_fail/origin `
  --output_dir ./images/process_image/20260616_output_1/equal_2_1_fail `
...
python ./process/04_image_crop_seg_v5_occluded_second_person.py `
  --input_dir ./images/process_image/20260616_output_1/equal_2_1_fail_4/debug/process_fail/origin `
  --output_dir ./images/process_image/20260616_output_1/equal_2_1_fail_5 `
  --model yolo11n.pt `
  --conf 0.1 `
  --padding 0.3 `
  --top_k 2 `
