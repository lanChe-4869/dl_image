...
def get_modal_id(url: str) -> str:
    """兼容三种输入：纯数字、搜索页 ?modal_id=、以及 https://www.douyin.com/note/{id}."""
    ...
    try:
        ...
        m = re.search(r"/(?:note|video)/(\d{10,})", parsed.path)
        if m:
            return m.group(1)
    except Exception:
        pass
...
def normalize_note_url(raw: str) -> str:
    """把 model_id / modal_id / 任意包含 id 的链接统一成 Douyin note URL。"""
    mid = get_modal_id(raw)
    if not mid:
        return str(raw or "").strip()
    return f"https://www.douyin.com/note/{mid}"
...
def uri_tail(uri_or_url: str) -> str:
    """从 tos-cn-i-0813/xxxx 或 URL 中提取图片对象 id，用于过滤当前帖子图片。"""
    x = str(uri_or_url or "")
    x = x.split("?", 1)[0]
    x = x.split("~", 1)[0]
...
def extract_aweme_image_info(data: dict) -> tuple[list[dict], set[str]]:
    """从 /aweme/detail JSON 中提取当前帖子的图片列表和 uri 白名单。"""
    images = data.get("images") or data.get("image_list") or []
    expected = []
    tails = set()
    ...
    for img in images:
        ...
        if tail:
            tails.add(tail)
        urls = []
        # url_list 通常是无水印/页面实际展示图；download_url_list 常带 tplv-dy-water-v2 水印。
...
def process_open_modal(page, modal_url: str, args, navigate: bool = False) -> dict:
    """处理一个确定的 note/model_id。

    关键修复：
    1) 先挂 response 监听，再打开 https://www.douyin.com/note/{id}；
    2) 捕获 /aweme/detail JSON，拿到当前 aweme_id 和 images[*].uri；
    3) 保存图片时只允许 URL 中包含当前帖子的图片 uri，避免混入推荐/相邻帖子。
    """
    modal_id = get_modal_id(modal_url) or "unknown_modal"
    note_url = normalize_note_url(modal_url)
    ...
    detail_data = None
    expected_images = []
    expected_uri_tails = set()

    def handle_response(response):
        nonlocal detail_data, expected_images, expected_uri_tails
        try:
            url = response.url
            headers = response.headers
            content_type = headers.get("content-type", "")

            # 先抓当前帖子的 detail JSON，后面用 images[*].uri 做白名单。
            if "/aweme/v1/web/aweme/detail" in url or "/aweme/v1/web/aweme/post" in url:
                try:
                    body = response.body()
                    data = json.loads(body.decode("utf-8", errors="ignore"))
                    # 兼容 {aweme_detail:{...}} / {aweme:{...}} / 直接 aweme 对象
                    cand = data.get("aweme_detail") or data.get("aweme") or data
                    if str(cand.get("aweme_id") or cand.get("group_id") or "") == str(modal_id):
                        detail_data = cand
                        expected_images, expected_uri_tails = extract_aweme_image_info(cand)
                        if expected_images and not args.expected_count:
                            args.expected_count = len(expected_images)
                except Exception:
                    pass
                return
    ...
    if navigate:
        print(f"    [GOTO NOTE] {note_url}")
        page.goto(note_url, wait_until="domcontentloaded", timeout=60_000)
    ...
    for step in range(args.max_steps):
        ...
        # 如果 detail JSON 来得较晚，用 JSON desc 更新标题/元信息即可；输出目录不再移动，避免路径混乱。
        if detail_data and not title:
            title = detail_data.get("desc") or detail_data.get("preview_title") or ""
        ...
        print(
            ...
            f"captured_total={len(captured)} new={len(new_candidates)} "
            f"saved={len(seen_hashes)} expected={args.expected_count or len(expected_images) or 0}"
        )

        saved_this_round = 0

        for cand in new_candidates:
            # 有白名单后，只保存属于当前 aweme images[*].uri 的 URL。
            if expected_uri_tails and not any(t in cand["url"] for t in expected_uri_tails):
                records.append({
                    "url": cand["url"],
                    "content_type": cand.get("content_type", ""),
                    "bytes": len(cand.get("body") or b""),
                    "saved": False,
                    "reason": "not_current_aweme_image_uri",
                })
                continue
        ...
        (out_dir / "post_meta.json").write_text(
            json.dumps({
                ...
                "input_url": modal_url,
                "modal_id": modal_id,
                "aweme_id": (detail_data or {}).get("aweme_id"),
                "title": title,
                "desc": (detail_data or {}).get("desc"),
                "expected_count": args.expected_count or len(expected_images),
                "expected_images": expected_images,
                "method": "note_url_detail_json_whitelist_network_capture",
        ...
        target = args.expected_count or len(expected_images)
        if target and len(seen_hashes) >= target:
            print(f"    [DONE] reached expected_count={target}")
            break
        ...
        # 只有当前帖子的 URI 白名单已知后，才继续翻图；即使翻到推荐帖，也会被白名单过滤。
...
def load_modal_urls_from_file(path: str, target_count: int | None = None) -> list[str]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            seen.add(mid)
            urls.append(normalize_note_url(mid))
        ...
        for scroll_idx in range(1, args.scroll_times + 1):
            ...
            for point_idx, point in enumerate(points, start=1):
                ...
                try:
                    ...
                    result = process_open_modal(search_page, modal_url, args, navigate=False)
    ...
    with sync_playwright() as p:
        ...
        for index, modal_url in enumerate(modal_urls, start=1):
            ...
            try:
                ...
                result = process_open_modal(page, modal_url, args, navigate=True)
...
python 05_download_douyin_note_images_fixed.py `
  --modal_urls_file images\single\20260812_single\0812_model_ids.txt `
  --output_dir images\single\20260812_single\images `
  --target_count 1000
